...
class ProtocolsPageService {
  ...
  getFilteredProtocolsByDate = (
    ...
    const startDateInMillisec =
      typeof startDate !== "number" ? Date.parse(startDate) : startDate;
    const endDateInMillisec =
      typeof endDate !== "number" ? Date.parse(endDate) : endDate;

    return dbFireStore
      .collection(`${collectionName}`)
      .where(`${dateField}`, ">=", startDateInMillisec)
      .where(`${dateField}`, "<=", endDateInMillisec)
      .get()
      .then((doc) => {
        const allSuitableDocs = [];
        doc.forEach((doc) => allSuitableDocs.push(doc.data()));
  ...
  getFilteredProtocolsAdvanced = (
    ...
    if (advancedFilterName === "createDate") {
      return this.getFilteredProtocolsByDate(
        basicFilterName,
        advancedFilterName,
        advancedFilterValue[0],
        advancedFilterValue[1],
        collectionName,
        userId
      );
    } else {
